...
def scrapeVideos(query, amount, lst):
    time.sleep(5)
    pyautogui.leftClick(960,80) # THIS CLICKS THE URL INPUT BAR
    ...
    time.sleep(1)
    pyautogui.click(350, 530) # THIS CLICKS THE FIRST VIDEO
    ...
    for num in range(0, int(amount)):
        ...
        pyautogui.leftClick(960,80) # THIS SELECTS THE LINK
        time.sleep(0.5)
        # pyautogui.hotkey('ctrl', 'c') # THIS COPIES THE LINK WINDOWS
        pyautogui.hotkey('command', 'c') # THIS COPIES THE LINK MAC
        time.sleep(.25)
        lst.append(pyperclip.paste()) # THIS PASTES THE LINK INTO A LIST
        time.sleep(1)
        pyautogui.click(1335,615) # THIS CLICKS THE ARROW TO SCROLL DOWN A TIKTOK
        time.sleep(1)

    pyautogui.click(39, 174) # THIS X'ES OUT OF THE TIKTOK SCREEN
